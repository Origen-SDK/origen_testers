module Testers
  module IGXLBasedTester
    class UltraFLEX < Base
      ...
      def set_code(code)
        RGen.log.warning 'Method set_code not supported for UltraFLEX! Find alternative solution.'
      ...
      def match_block(timeout, options = {}, &block)
        ...
        match_conditions = RGen::Utility::BlockArgs.new
        fail_conditions = RGen::Utility::BlockArgs.new

        # yield object to calling routine to get populated with blocks
        if block.arity > 0
          yield match_conditions, fail_conditions
        else
          # for backwards compatibility with RGen core call to match_block

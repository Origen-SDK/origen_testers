module Testers
  class CallbackHandlers
    ...
    # Listen for a pattern with .atp or .atp.gz extension.  If found then compile the fileand kill the 'rgen g' command
    def before_pattern_lookup(requested_pattern)
      ...
      if patname.to_s.match(/.atp/)
        if patname.extname == '.atp' || patname.extname == '.gz'
          ...
          matches = Dir.glob("#{RGen.root}/**/#{patname}")
        ...
        # Return false so the RGen generate command stops
        return false
      end
      true
    end
    # Instantiate an instance of this class immediately when this file is required, this object will
    # then listen for the remainder of the RGen thread

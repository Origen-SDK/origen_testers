      ...
      class Builder
        ...
        def render(options)
          manifest[:flows].each do |name, flow|
            ...
            compile_options = {
              action:           :compile,
              files:            "#{RGen.root!}/lib/testers/smartest_based_tester/v93k/templates/template.flow.erb",
              output_file_name: "#{name}.flow",
              output_sub_dir:   'testflow',
              options:          { program: flow_file }
            }.merge(options)

            RGen.app.runner.launch(compile_options)

            if pm_file
              compile_options = {
                action:           :compile,
                files:            "#{RGen.root!}/lib/testers/smartest_based_tester/v93k/templates/template.pmfl.erb",
        ...
        def find_file(file)
          RGen.file_handler.clean_path_to(file, default_dir: @manifest_dir)
        end

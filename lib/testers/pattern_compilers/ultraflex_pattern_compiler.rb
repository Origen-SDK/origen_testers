module Testers
  module PatternCompilers
    class UltraFlexPatternCompiler
      ...
      LINUX_PATTERN_COMPILER = "#{RGen.root!}/bin/latest/bin/atpcompiler"
      # LINUX_PATTERN_COMPILER = "#{RGen.root!}/bin/latest/bin/atpcompiler"
      ...
      ATPC_SETUP = "#{RGen.root!}/bin/latest/etc/atpcrc.csh"
      ...
      def initialize(id, options = {})
        ...
        @user_options = {
          ...
          reference_directory: nil, # optional: will be set to @path or RGen.app.config.pattern_output_directory
        ...
        if RGen.app.config.lsf.project.nil? || RGen.app.config.lsf.project.empty?
          puts 'LSF is not set at RGen.app.config.lsf.project, changing to local compilation'
      ...
      def find_jobs(path = @path)
        ...
        set_reference_directory
        RGen.profile 'Linux pattern compiler finds patterns' do
        ...
        RGen.profile 'Linux pattern compiler creates jobs' do
          @files.each do |f|
      ...
      def run(list = nil, options = {})
        fail "Error: the tester #{RGen.tester} is not an Ultrflex tester,exiting..." unless is_ultraflex?
        fail "Error: application #{RGen.app.name} is running on Windows, to run the pattern compiler you must be on a Linux machine" if running_on_windows?
        ...
        if list.nil?
          ...
          @jobs.each do |job|
            ...
            if job.location == :lsf
              RGen.app.lsf.submit(ATPC_SETUP + '; ' + job.cmd)
            else
              RGen.profile "Linux pattern compiler compiles pattern #{job.pattern}" do
                system job.cmd
              end
            end
          end
      ...
      def set_reference_directory
        if @user_options[:reference_directory].nil?
          # Nothing passed for reference directory so set it to RGen.app.config.pattern_output_directory if valid
          if File.directory? RGen.app.config.pattern_output_directory
            @user_options[:reference_directory] = Pathname.new(RGen.app.config.pattern_output_directory)
          elsif @path
            if @path.directory?

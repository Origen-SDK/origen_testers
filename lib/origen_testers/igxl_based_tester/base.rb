module OrigenTesters
  module IGXLBasedTester
    ...
    class Base
      ...
      #fullconfigpath example ==> "/rgen/product_folder/CurrentConfig.txt"

        puts "importing Testerconfig #{testconfigname}..."
        slotnum = Struct.new(:slot, :instrument, :idprom)
        @testerconfig[testconfigname] ||= Hash.new
      ...
      def get_tester_instrument(testconfigname,slot)
        ...
        return nil  #if no corresponding slot
      end

      def get_instrument_slots(testconfigname,instrument)  #testconfigname example "CurrentConfig.txt"
        @slots = []
        @testerconfig[testconfigname].each_with_index do |element,index|
      ...
      #fullchanmappath example ==> "/rgen/product_folder/Chans_FT.txt"
        ...
        @channelmap[chanmapname] ||= Hash.new
        File.open(chanmap_file, 'r').each_line.with_index do |line, index|
          ...
          if index == 5
            ...
            for sitenum in 0..@max_site
              @channelmap[chanmapname][sitenum] ||= Hash.new
            end
          end
          if index > 5            
